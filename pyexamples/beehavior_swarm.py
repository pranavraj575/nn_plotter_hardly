...
arch = [
    ...
    to_input(pathfile=os.path.join(os.path.dirname(__file__), 'optic_flow_39.png'),
             ...
             invert=True),
    to_Conv("conv1invis", xlabel=initial_img_channels, ylabel=240, zlabel='~'*8 + str(320),
            offset="(2,0,0)", to="(invis-east)",
            width=len_map(initial_img_channels), height=len_map(240), depth=len_map(320),
            opacity=0,
            invisible=True,
            ),
    to_dotted_diags('invis', 'conv1invis'),
    ...
    to_Pool("pool1invis", xlabel=initial_img_channels, ylabel=60, zlabel='~'*8 + str(80),
    ...
    to_Pool("pool2invis", xlabel=16, ylabel=9, zlabel='~'*4 + str(13),
            offset="(0.5,0,0)", to="(conv2-east)",
            width=len_map(16), height=len_map(9), depth=len_map(13),
            opacity=0,
            invisible=True,
            ),
    to_dotted_diags('conv2', 'pool2invis'),
    ...
    to_ConvConvRelu("linearinvis", zlabel='~'*8 + str(64),
                    offset="(1,0,0)", to="(flatten-east)",
                    width=len_map(0), height=len_map(0), depth=len_map(64),
                    opacity=0,
                    invisible=True,
                    ),
    to_connection("flatten-east", "linearinvis-west"),
    to_ConvConvRelu("linear", zlabel='~'*8 + str(64),
                    offset="(1,0,0)", to="(flatten-east)",
                    width=len_map(0), height=len_map(0), depth=len_map(64),
                    caption="{Linear}",
                    ),
    to_Conv("outputinvis", zlabel=2,
            offset="(1,0,0)", to="(linear-east)",
            width=len_map(0), height=len_map(0), depth=len_map(2),
            opacity=0,
            invisible=True,
            ),
    to_connection("linear-east", "outputinvis-west"),
